...
/************************************************************************
公寓楼房更新模块，通过输入公寓号进行精确查询，之后修改对应数据即可完成修改
**************************************************************************/
namespace dormitory_manage
{
    public partial class house_update : Form
    {
        ...
        private bool int_to_bool(Object i)
        {
            if (i == null)
            {
                return false;
        ...
        {
            if (int_to_bool(Sqlhelper.getScalar(string.Format("select 楼层数 from 公寓楼房 where 公寓号='{0}'", house_num.Text))))
            {
